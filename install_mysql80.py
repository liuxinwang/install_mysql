...
def get_conf_file():
  if len(sys.argv) == 1:
    logger.error('配置文件不存在. 例如：python install_mysql80.py conf.ini')
    sys.exit()
  return sys.argv[1]
...
def download_mysql(mysql_version, md5_value):
  mysql_tar_filename \
      = "mysql-%s-linux-glibc2.12-x86_64.tar.xz" % mysql_version
  mysql_download_url \
      = "https://dev.mysql.com/get/Downloads/MySQL-8.0/%s" % mysql_tar_filename
...
def unzip_mysql(mysql_version):
  mysql_tar_filename = "mysql-%s-linux-glibc2.12-x86_64.tar.xz" % mysql_version
  create_mysql_dir = "/opt/mysql"
  os.system("mkdir %s" % create_mysql_dir)
  os.system("tar -xvJf " + mysql_tar_filename + " -C %s/" % create_mysql_dir)
...
def dir_definition(db_port):
  ...
  os.system("cp my80.cnf %s/my%s.cnf" % (data_dir, db_port))
...
def modify_my_cnf_para(db_port):
  os.system(
    """sed -i 155,158 s/^#//g /data/mysql/mysql%s/my%s.cnf"""
    % (db_port, db_port))
